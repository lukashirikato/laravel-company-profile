...
<div class="min-h-screen bg-dark py-8">
    <div class="max-w-2xl mx-auto px-4">
        ...
        <div class="mb-8">
            ...
            <p class="text-dark/40">Scan QR code member untuk check-in otomatis</p>
        </div>

        <!-- SCANNER CARD -->
        <div class="bg-dark rounded-xl p-8 border border-gray-700 mb-6">
            ...
            <div id="camera-container" class="mb-6 hidden">
                ...
                <button onclick="stopCamera()" class="mt-3 w-full bg-secondary hover:bg-secondary text-white font-semibold py-2 rounded-lg transition">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-semibold text-dark/30 mb-2">Scan QR Code atau Input Token</label>
                    <input type="text" 
                           id="qr-input" 
                           placeholder="Paste QR token hasil scan di sini..."
                           class="w-full px-4 py-3 bg-dark border border-gray-600 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-red-600"
                           autofocus>
                </div>

                <div>
                    <label class="block text-sm font-semibold text-dark/30 mb-2">Program (Opsional)</label>
                    <input type="text" 
                           id="program-input" 
                ...
                <button onclick="submitScan()" class="w-full bg-secondary hover:bg-secondary text-white font-semibold py-3 rounded-lg transition flex items-center justify-center gap-2">
        ...
        <div id="result-area" class="hidden mb-6">
            ...
            <button onclick="resetScan()" class="mt-4 w-full bg-dark hover:bg-gray-600 text-white font-semibold py-2 rounded-lg transition">
...
<script>
        ...
        resultMsg.innerHTML = `
            <div class="text-center">
                ...
                <div class="bg-green-900/20 rounded-lg p-4 my-4 space-y-2 text-left">
                    <p class="text-dark/30"><strong>Member:</strong> ${data.member_name} (${data.member_id})</p>
                    <p class="text-dark/30"><strong>Program:</strong> ${data.program}</p>
                    <p class="text-dark/30"><strong>Jam Masuk:</strong> ${data.check_in_time} (${data.check_in_date})</p>
                    <p class="text-dark/30"><strong>Package:</strong> ${data.package_name}</p>
                    ...
                    <div class="pt-3 border-t border-green-700">
                        ...
                        <div class="mt-2 w-full bg-green-900/50 rounded-full h-2">
                            <div class="bg-grounded-green/200 h-full rounded-full" style="width: ${(data.remaining_quota / data.total_quota) * 100}%"></div>
                        </div>
                    </div>
                </div>
    ...
    function logScan(data, status) {
        ...
        logEntry.className = `border-l-4 ${status === 'success' ? 'border-green-600' : 'border-red-600'} bg-dark/50 p-3 rounded text-sm`;
        logEntry.innerHTML = `
    ...
    function alertMessage(message, type = 'info') {
        const bgClass = type === 'error' ? 'bg-secondary' : 'bg-primary';
