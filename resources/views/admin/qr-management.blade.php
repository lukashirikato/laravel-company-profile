    ...
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <p class="text-dark/40">Manage, monitor, and troubleshoot member QR codes</p>
            </div>
            <a href="{{ route('admin.dashboard') ?? '#' }}" class="bg-dark hover:bg-dark text-white px-6 py-2 rounded-lg transition flex items-center gap-2">
        ...
        <div class="grid md:grid-cols-4 gap-4 mb-8">
            <div class="bg-gradient-to-br from-green-900/20 to-green-800/20 border border-green-700 rounded-xl p-6">
                <div class="flex items-center justify-between">
                    ...
                    <i class="ri-qr-code-line text-4xl text-accent opacity-50"></i>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-blue-900/20 to-blue-800/20 border border-blue-700 rounded-xl p-6">
                <div class="flex items-center justify-between">
                    ...
                    <i class="ri-check-double-line text-4xl text-primary opacity-50"></i>
                </div>
            </div>

            <div class="bg-gradient-to-br from-purple-900/20 to-purple-800/20 border border-secondary rounded-xl p-6">
                <div class="flex items-center justify-between">
                    <div>
                    ...
                    <i class="ri-close-circle-line text-4xl text-primary opacity-50"></i>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-yellow-900/20 to-yellow-800/20 border border-yellow-700 rounded-xl p-6">
                <div class="flex items-center justify-between">
                    ...
                    <i class="ri-alert-line text-4xl text-springs-ivy opacity-50"></i>
                </div>
            </div>
        </div>

        <!-- SEARCH & FILTER -->
        <div class="bg-dark rounded-xl p-6 border border-gray-700 mb-8">
            <div class="grid md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm text-dark/40 mb-2">Search by Name or Email</label>
                    <input type="text" id="search-input" placeholder="Member name or email..." 
                        class="w-full bg-dark border border-gray-600 rounded-lg px-4 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-primary">
                </div>
                <div>
                    <label class="block text-sm text-dark/40 mb-2">Filter by Status</label>
                    <select id="filter-status" class="w-full bg-dark border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-primary">
        ...
        <!-- MEMBERS LIST -->
        <div class="bg-dark rounded-xl border border-gray-700 overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead>
                        <tr class="bg-dark/50 border-b border-gray-700">
                            <th class="text-left px-6 py-4 text-dark/30 font-semibold text-sm">Member ID</th>
                            <th class="text-left px-6 py-4 text-dark/30 font-semibold text-sm">Name</th>
                            <th class="text-left px-6 py-4 text-dark/30 font-semibold text-sm">Email</th>
                            <th class="text-left px-6 py-4 text-dark/30 font-semibold text-sm">QR Status</th>
                            <th class="text-left px-6 py-4 text-dark/30 font-semibold text-sm">Generated</th>
                            <th class="text-left px-6 py-4 text-dark/30 font-semibold text-sm">Actions</th>
                        </tr>
                    </thead>
                    <tbody id="members-table-body">
                        <!-- Will be populated by API/JavaScript -->
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-dark/40">
        ...
        <!-- BULK ACTIONS -->
        <div class="mt-8 bg-dark rounded-xl p-6 border border-gray-700">
            <h3 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
                <i class="ri-settings-line text-primary"></i> Bulk Actions
            </h3>
            <div class="flex flex-col md:flex-row gap-3">
                <button onclick="generateQRForAll()" class="flex-1 bg-primary hover:bg-secondary text-white font-semibold py-2 rounded-lg transition flex items-center justify-center gap-2">
                    <i class="ri-qr-code-line"></i> Generate Missing QR Codes
                </button>
                <button onclick="enableAllQR()" class="flex-1 bg-accent hover:bg-springs-ivy text-white font-semibold py-2 rounded-lg transition flex items-center justify-center gap-2">
                    <i class="ri-check-line"></i> Enable All QR Codes
                </button>
                <button onclick="disableAllQR()" class="flex-1 bg-secondary hover:bg-secondary text-white font-semibold py-2 rounded-lg transition flex items-center justify-center gap-2">
...
<div id="member-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 p-4">
    <div class="bg-dark rounded-xl shadow-lg max-w-md w-full p-6 relative border border-gray-700">
        <button onclick="closeMemberModal()" class="absolute top-3 right-3 text-cream0 hover:text-dark/30 text-2xl">×</button>
        ...
        <div id="modal-content" class="space-y-4">
            ...
            <div class="bg-white rounded-lg p-4 flex justify-center">
                <div id="modal-qr" class="w-64 h-64 flex items-center justify-center">
                    <i class="ri-qr-code-line text-6xl text-dark/30"></i>
                </div>
            </div>

            <!-- Member Details -->
            <div>
                <p class="text-sm text-dark/40 mb-1">Member Info</p>
                <div class="bg-dark rounded-lg p-3 space-y-1">
                    <p class="text-white font-semibold" id="modal-name"></p>
                    <p class="text-dark/30 text-sm" id="modal-email"></p>
                    <p class="text-dark/40 text-sm">Member ID: <span id="modal-id" class="font-mono"></span></p>
                </div>
            </div>

            <!-- QR Status -->
            <div>
                <p class="text-sm text-dark/40 mb-1">QR Code Status</p>
                <div class="bg-dark rounded-lg p-3">
                    <p class="text-white font-semibold" id="modal-status"></p>
                    <p class="text-dark/40 text-sm mt-1">Generated: <span id="modal-generated"></span></p>
                </div>
            </div>

            <!-- QR Token -->
            <div>
                <p class="text-sm text-dark/40 mb-1">QR Token</p>
                <div class="bg-dark rounded-lg p-3">
                    <p class="text-dark/30 font-mono text-xs break-all" id="modal-token"></p>
                </div>
            </div>

            <!-- Modal Actions -->
            <div class="flex gap-2">
                <button onclick="downloadQRFromModal()" class="flex-1 bg-primary hover:bg-secondary text-white font-semibold py-2 rounded-lg transition text-sm">
                    <i class="ri-download-line mr-1"></i>Download
                </button>
                <button onclick="regenerateQRFromModal()" class="flex-1 bg-springs-ivy hover:bg-yellow-700 text-white font-semibold py-2 rounded-lg transition text-sm">
                    <i class="ri-refresh-line mr-1"></i>Regenerate
                </button>
                <button onclick="toggleQRFromModal()" id="modal-toggle-btn" class="flex-1 bg-secondary hover:bg-secondary text-white font-semibold py-2 rounded-lg transition text-sm">
...
<script>
    ...
    function renderMembersTable(data) {
        ...
        tbody.innerHTML = data.map(member => {
            ...
            return `
                <tr class="border-b border-gray-700 hover:bg-dark/30 transition">
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                        <span class="text-dark/40 text-sm">${member.email}</span>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                        <span class="text-dark/40 text-sm">${member.qr_generated_at ? new Date(member.qr_generated_at).toLocaleDateString() : '-'}</span>
                    </td>
                    <td class="px-6 py-4">
    ...
    function openMemberModal(memberId) {
        ...
        if (member.qr_token) {
            ...
            if (member.qr_active) {
                ...
                button.className = 'flex-1 bg-secondary hover:bg-secondary text-white font-semibold py-2 rounded-lg transition text-sm';
            } else {
                document.getElementById('modal-status').textContent = '✗ Inactive';
                button.textContent = '✓ Enable QR';
                button.className = 'flex-1 bg-accent hover:bg-springs-ivy text-white font-semibold py-2 rounded-lg transition text-sm';
            }
